...
function goto1(index,beforeCallbak,afterCallback){
  ...
  beforeCallback && beforeCallback(currentIndex);
  papers.eq(index).css({
    zIndex : 100,
    ...
    afterCallback && afterCallback(currentIndex); 
  });
...
for(var i=0; i<bgImgSize.length; i++){
  ...
  if(bgRatio>=winRatio){
    papers.eq(i).children('.bg').addClass('v');
  }else{
    papers.eq(i).children('.bg').addClass('h');
  }
}
...
function beforeCallback(index){
  index = index + 1;
  animations['p'+index] && animations['p'+index](false);
}
function afterCallback(index){
  index = index + 1;
  animations['p'+index] && animations['p'+index]();
}
