(function ($) {
    ...
    function startTimeliner(options) {
        var settings = { timelineContainer: options["timelineContainer"] || "#timelineContainer",
            startState: options["closed"] || "closed",
        ...
        function openEvent(eventHeading, eventBody) {
            $(eventHeading).removeClass("closed").addClass("open").animate({ fontSize: settings.fontOpen },
            settings.baseSpeed); $(eventBody).show(settings.speed * settings.baseSpeed)
        }
        ...
        if ($(settings.timelineContainer).data("started")) {
            ...
            if (settings.startState === "closed") {
                ...
                openEvent($(settings.timelineContainer + " " + ".timelineMinor dt a"),
                $(settings.timelineContainer + " " + ".timelineEvent"))
            }
            ...
            $(settings.timelineContainer).on("click", ".timelineMajorMarker", function () {
                ...
                if (numEvents > numOpen) {
                    openEvent($(this).parents(".timelineMajor").find("dt a", "dl.timelineMinor"),
                    $(this).parents(".timelineMajor").find(".timelineEvent"))
                }
                else {
                    closeEvent($(this).parents(".timelineMajor").find("dl.timelineMinor a"),
                    $(this).parents(".timelineMajor").find(".timelineEvent"))
                }
            });
            $(settings.timelineContainer + " " + ".expandAll").click(function () {
                if ($(this).hasClass("expanded")) {
                    closeEvent($(this).parents(settings.timelineContainer).find("dt a", "dl.timelineMinor"),
                    $(this).parents(settings.timelineContainer).find(".timelineEvent"));
                    $(this).removeClass("expanded").html(settings.expandAllText)
                }
                else {
                    openEvent($(this).parents(settings.timelineContainer).find("dt a", "dl.timelineMinor"),
                    $(this).parents(settings.timelineContainer).find(".timelineEvent"));
